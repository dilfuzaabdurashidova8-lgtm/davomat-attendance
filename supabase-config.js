...
window.SB.pushAll = async function(silent){
  ...
  try{
    ...
    if(students.length)
      await this.client.from('students').upsert(students.map(s => ({id: s.id, full_name: s.name, university_id: s.uniId, group_name: s.group || '', phone: s.phone || '', days: (s.days || []).join(',')})));
    ...
    return true;
  }catch(e){ console.warn('pushAll:', e); if(!silent && typeof toast === 'function') toast('❌ ' + (e.message || 'sync error')); return false; }
...
window.SB.saveAttendance = async function(date, dayRec){
  ...
  try{
    ...
    if(usedStudents.length)
      await this.client.from('students').upsert(usedStudents.map(s => ({id: s.id, full_name: s.name, university_id: s.uniId, group_name: s.group || '', phone: s.phone || '', days: (s.days || []).join(',')})));
...
window.SB.load = async function(){
  ...
  try{
    ...
    (stus || []).forEach(s => mapS.set(s.id, {id: s.id, name: s.full_name, uniId: s.university_id, group: s.group_name || '', phone: s.phone || '', days: s.days ? String(s.days).split(',').map(Number).filter(n => n >= 1 && n <= 7) : []}));
